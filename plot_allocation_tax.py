...
def plot_allocation_tax(results: dict, out: Path, N: int = 131072, L: int = 40):
    ...
    bars_math = ax.bar(pos_math, math_vals, width=width, color=c_math, edgecolor="black", linewidth=0.6, label="Mathematical Payload", zorder=3, hatch=HATCH_PATTERNS[0])
    bars_serial = ax.bar(pos_serial, serialized_vals, width=width, color=c_serial, edgecolor="black", linewidth=0.6, label="Serialized Size", zorder=3, hatch=HATCH_PATTERNS[1])
    bars_rss = ax.bar(pos_rss, rss_vals, width=width, color=c_rss, edgecolor="black", linewidth=0.6, label="RSS (Resident)", zorder=3, hatch=HATCH_PATTERNS[2])
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=Path("plots/allocation_tax.pdf"),
