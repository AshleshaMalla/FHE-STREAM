> # ⚠️ RETRACTED — MARKER-MODE AMPLIFICATION TABLE SUPERSEDED (2026-07-30)
>
> **Part 1 (the five-kernel DRAM-amplification table) of this document is retracted.**
> The marker-mode amplification factors (2.57×–6.23×) were produced by an unreliable
> measurement chain and could not be independently reproduced:
> - The "clean" `SEQ_ADD` source CSV was produced with the **banned `-C` flag in marker
>   mode**, which collapsed all 256 threads onto CPU 0 (every column labeled `HWThread 0`);
>   the tidy single number was the collapse artifact, not a valid measurement.
> - The published 2.57× actually traced to a *different* file whose 110.5 GB total spanned
>   ~4 marker iterations but was divided by a single-iteration logical baseline
>   (iteration-count accounting error).
> - Fresh marker-mode `SEQ_ADD` re-runs on the current binary gave **0.19× / 0.00× / 0.49×**
>   (thread collapse, socket-1 zeros) — non-reproducible.
> - Four of five kernels were **socket-summed** from two independent 128-thread passes, each
>   of which processes the *entire* B=512 working set → the sum **double-counted** (~2×),
>   producing implied bandwidths that **exceeded the 858 GiB/s hardware peak** (physically
>   impossible).
>
> **Superseded by:** `results/validation_summary.md`, rebuilt from whole-process
> (non-marker) LIKWID DF-counter measurements using a SetUp-cancelling differential.
> This file is retained **for the record only** — do **not** cite Part 1 numbers below.
> (Part 2, the CT correlation, was never marker-derived and is carried forward unchanged
...
**These are single-shot aggregate measurements — no error bars or confidence intervals
may be attached, and this limitation must carry through into the paper's methodology text
and figure design as an explicitly stated caveat.** Each factor is one accumulated DRAM
byte total per kernel per direction: the LIKWID DFC memory-volume counter is socket-scoped
(one scalar per socket, attributed to a single representative thread — verified in the
marker CSVs, where the per-socket total lands on HWThread 0/128 and all other threads read
0), and marker regions accumulate destructively (a call count > 1 sums counter deltas
in place and retains only the total plus the count, never the per-call values). Some
threads did record call counts of 2–4, but these yield only a per-call *mean*
(`total / count`), not a spread — the individual per-call samples do not exist in the data.
Combined with the fact that no kernel/direction has a duplicate run on disk, there is no
dataset from which a variance, standard deviation, or CI could be derived without a fresh
rerun. Report the five factors as single measurements.
